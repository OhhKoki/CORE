...
悲观锁（Pessimistic Locking）是一种数据库或多线程编程中的锁机制，其核心思想是假设数据会被多个~~进程或~~线程同时修改，因此在操作数据时，会采取锁的方式，确保其他线程或进程不能对数据进行并发修改，直到当前操作完成。
...
CAS（Compare-And-Swap）是一种用于在多线程环境下实现同步功能的机制。CAS 有三个操作数：内存地址、预期数值、新值。

CAS 的实现逻辑是：将内存地址处的数值与预期数值进行比较，若相等，则将内存地址处的值替换为新值，若不相等，则不做任何操作。
...
CAS操作的优点：无需加锁，避免了线程阻塞和上下文切换的开销。
...
**没有使用 CAS 的情况下（也不使用锁），存在线程安全问题**

在这个例子中，我们有一个简单的计数器类，多个线程并发地更新计数器，但没有使用 CAS 来保证线程安全。由于多个线程可能同时读取和更新`count`值，导致数据竞争和线程安全问题。
...
运行这个程序时，由于多个线程并发修改`count`值，没有使用 CAS 保护，所以最终的计数结果往往小于预期的1000000。这是因为`count++`操作并不是原子的，多个线程在读取和更新`count`时可能会覆盖彼此的修改，导致丢失更新。



**使用 CAS 对上述案例进行改造，解决线程安全问题**
