...
AQS 是 Java 中用于实现同步器的基础框架，它利用原子操作和队列来管理线程间的同步，提供了一个高效、可扩展的同步机制。
...
- **FIFO 等待队列（Node 链表）**：当线程无法获取锁时，它会被加入一个等待队列中，按照先来先服务的顺序等待唤醒。
  - Node 类型的双向链表
...
多个线程同时采用 CAS 修改 AQS 的 state 属性时，只有一个线程能修改成功（获取到锁），CAS 修改失败的线程：
...
- 在这一阶段，Java 类加载器（ClassLoader）将类的字节码从文件系统或者网络加载到内存中。
...
2. **方法区（Method Area）**
   ...
   - 用途： 方法区存储被加载的类信息、方法信息、常量池、静态变量等。它是一个共享内存区域，所有线程都可以访问它。方法区有时也被称为“永久代”（在 Java 8 之前），但在 Java 8 之后，方法区已被 Metaspace 替代。

   - Metaspace： 从 Java 8 开始，Metaspace 取代了永久代，存储类的元数据。与永久代不同，Metaspace 不受 JVM 启动时指定的堆大小限制，而是根据操作系统的可用内存动态调整大小。Metaspace 的默认大小取决于操作系统的资源。
