...
主键一般使用int类型（4个字节），指针（指针在InnoDB中占6个字节）。也就是说一个节点占用10个字节。

而这些节点是存在page中的，一个page大小为16kb

那么，一个page中可以存的节点数量为：

1. 一个节点的大小为：主键值（4b）+ 指针（6b）= 10b
2. 一个 page 的大小为：16kb=(16 * 1024)b
3. 那么一个 page 中可以存的节点数为：(16 * 1024) / 10 = 1638 个节点

那么一个三层的 B+树 可以有多少个节：1638 * 1638 * 16 = 42928704



如果是使用 BigInteger 为主键，

1. 单个节点占用的字节数：8 + 6 = 14
2. 一个page能存放的节点数：(16*1024) / 14 = 1170
3. 三层的 B+ 树的总节点数：1170 * 1170 * 16 = 21902400
   - 假设一行数据占用 1kb，那么 一个 page 就是 16 行数据
...
- COUNT(1) 计算所有行数，包括 NULL 值，性能与 C OUNT(*) 没有区别。
