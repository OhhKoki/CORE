...
如果是直接创建线程，然后使用 ThreadLocal（不使用线程池）的话，ThreadLocal 是不存在线程泄露问题的。当线程执行结束，那么线程就要销毁，线程中的 threadlocals 属性也随之销毁，那么对应的 Entry 对象当然也会被销毁了。
...
==ReentrantLock 中 tryLock() 和 ock() 的区别？==
