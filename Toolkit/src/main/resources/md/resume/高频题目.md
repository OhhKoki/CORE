...
![image-20250624145131054](./assets/img37.png)
...
Spring IOC（Inversion of Control，控制反转）是 Spring 框架的核心概念之一，它指的是通过依赖注入（DI）机制，将对象的创建和管理的控制权交给 Spring 容器，而不是由程序员手动创建和管理对象。这样可以降低类之间的耦合度，提高系统的可维护性和可扩展性。

主要概念：

1. **IOC 容器**：
   - Spring 容器负责创建、管理和销毁对象。它通过读取配置文件（如 XML 配置或注解配置）来定义哪些对象需要管理，并自动完成对象之间的依赖注入。

2. **依赖注入（DI）**：
   - 依赖注入是 IOC 的核心，指的是通过构造函数、字段或方法的注入方式，将其他类的实例传递给当前类，而不是在当前类内部自己创建。这可以通过：
     - **构造器注入**：通过类的构造方法注入依赖。
     - **Setter 注入**：通过 setter 方法注入依赖。
     - **字段注入**：通过字段直接注入依赖。

3. **Bean 定义与管理**：
   - Spring 将被管理的对象称为 **Bean**。每个 Bean 都有唯一的标识符（通常是类的名称或在配置文件中指定的名称），并且在 Spring 容器中由容器进行生命周期管理。

4. **控制反转的实现**：
   - Spring 容器会在应用启动时创建并初始化所有 Bean，并在需要时提供这些 Bean 的实例，通过依赖注入将这些 Bean 注入到需要使用它们的类中。

工作原理：

1. **Bean 定义**：使用 XML 配置或注解（如 `@Component`、`@Service`、`@Repository`、`@Controller`）定义一个 Bean。
2. **Bean 创建**：Spring 容器会在应用启动时读取这些定义并创建相应的对象。
3. **依赖注入**：当某个类需要使用其他类时，Spring 会自动注入所需的 Bean 实例。

优点：

- **解耦**：类之间不直接依赖于具体的实现，而是依赖于接口或抽象类，增强了模块的独立性。
- **灵活性**：可以方便地切换实现，使用不同的 Bean 配置，而不需要修改客户端代码。
- **可维护性**：简化了对象的创建和管理，减少了硬编码，提高了系统的可测试性和可维护性。
...
Spring IOC 通过控制反转和依赖注入机制，解耦了对象之间的依赖关系，使得代码更加灵活和易于测试。通过配置 Bean 并自动注入依赖，开发者可以专注于业务逻辑的实现，减少手动管理对象生命周期的繁琐。



#### AOP

Spring AOP（面向切面编程）是Spring框架中的一个核心模块，提供了以声明性方式将横切关注点（如日志、事务管理、性能监控等）分离到独立的模块中的功能。它允许你在不修改源代码的情况下，向现有的类添加功能。

##### 1. AOP的基本概念
- **横切关注点（Cross-cutting Concern）**：是指那些与核心业务逻辑无关的功能（如日志、安全、事务等），这些功能会影响到多个模块或类。
- **切面（Aspect）**：定义了横切关注点的实现，包括“通知”和“切点”。
- **通知（Advice）**：是切面中定义的具体操作或功能（如日志记录、事务控制），它会在特定的连接点上执行。
- **切点（Pointcut）**：定义了通知应该在哪些连接点（方法执行）上执行。切点表达式用于指定目标方法的位置。
- **连接点（Joinpoint）**：程序执行中的一个点，可以是方法的执行、构造函数的调用等。
- **目标对象（Target Object）**：被代理的对象，即我们要增强的对象。
- **代理（Proxy）**：通过AOP生成的代理对象，目标对象的代理，可以在执行目标方法前后添加额外的操作。
...
- **通知类型（Advice）**：
  ...
  - **环绕通知（Around）**：可以在目标方法执行前后执行，并且可以控制是否执行目标方法。
  - **异常通知（After-Throwing）**：当目标方法抛出异常时执行。
  - **返回通知（After-Returning）**：目标方法正常执行完后执行。

- **切点表达式（Pointcut Expression）**：定义哪些方法会被切面增强。
  - 通过注解（如`@Before`, `@After`）或者通过XML配置来定义。

##### 3. AOP的工作原理
Spring AOP通过动态代理技术来实现AOP的功能。Spring提供了两种类型的代理方式：
- **JDK动态代理**：基于接口的代理，仅适用于接口实现类。
- **CGLIB代理**：基于类的代理，适用于没有接口的类，通过子类化目标类来实现代理。

在执行目标方法时，代理对象会执行与目标方法关联的通知，最终会将调用传递给目标对象。

##### 4. AOP的应用场景
- **事务管理**：通过AOP可以在方法执行前后自动管理事务，避免代码中频繁写入事务相关代码。
- **日志记录**：可以使用前置通知和后置通知来记录方法调用的日志，简化代码。
- **权限控制**：可以在方法执行前检查用户权限，统一实现权限校验。
- **性能监控**：可以通过环绕通知在方法调用前后记录执行时间，用于性能分析。

##### 5. Spring AOP的注解支持
Spring提供了注解来简化AOP的配置：
- **@Aspect**：标注一个类为切面。
- **@Before**：前置通知，指定方法在目标方法执行前执行。
- **@After**：后置通知，指定方法在目标方法执行后执行。
- **@AfterReturning**：返回通知，指定方法在目标方法执行成功后执行。
- **@AfterThrowing**：异常通知，指定方法在目标方法抛出异常后执行。
- **@Around**：环绕通知，指定方法围绕目标方法执行，可以控制目标方法的执行。
...
public class LoggingAspect {
    ...
    @After("execution(* com.example.service.*.*(..))") // 后置通知
...
##### 7. 总结
Spring AOP提供了一个灵活的方式来分离横切关注点（如日志、事务、安全等），并增强程序的模块化。它使得开发者能够在不修改业务逻辑代码的前提下，实现功能的扩展和维护。通过注解和配置，可以轻松定义切面和通知，增强了代码的可重用性和可维护性。



#### 什么是自动装配

Spring的自动装配是一种自动注入依赖关系的机制，它简化了开发者在Spring容器中手动配置Bean的工作。通过自动装配，Spring框架可以根据Bean的类型或名称自动匹配合适的依赖项，自动完成对象的注入，减少了配置的复杂度。Spring提供了几种不同的自动装配方式，下面是核心要点：

1. **自动装配的方式**：
   - **byType**：根据Bean的类型进行注入。在这种方式下，Spring会查找匹配的类型，并注入对应的Bean。如果有多个同类型的Bean，Spring会抛出异常。
   - **byName**：根据Bean的名称进行注入。Spring会通过属性的名称匹配容器中已有的Bean名称，并将对应的Bean注入到该属性中。
   - **constructor**：通过构造函数注入依赖。Spring会根据构造函数的参数类型自动选择匹配的Bean进行注入。
   - **@Autowired注解**：这是Spring提供的一种注解方式，用于标注需要自动装配的Bean。可以应用在字段、构造函数或者setter方法上。

2. **@Autowired的使用**：
   - **字段注入**：直接在字段上使用`@Autowired`注解，Spring会自动装配该字段所需的Bean。
   - **构造器注入**：可以在构造器上使用`@Autowired`注解，Spring会使用构造函数注入Bean。通常推荐使用构造器注入，因为它能确保Bean在创建时完成所有依赖的注入。
   - **setter方法注入**：可以在setter方法上使用`@Autowired`注解，Spring会通过调用setter方法来进行依赖注入。

3. **自动装配的优先级**：
   - **默认情况下**，Spring会根据类型来进行自动装配，但是如果类型不唯一，Spring会抛出异常。可以通过`@Primary`注解来标记优先注入的Bean，或者使用`@Qualifier`注解来指定具体的Bean名称。
   
4. **@Autowired与@Qualifier的配合使用**： 
   当有多个同类型的Bean时，可以通过`@Qualifier`指定具体的Bean名称来避免冲突，确保准确注入所需的Bean。

5. **自动装配的依赖管理**：
   - Spring容器会根据配置文件或注解自动注入依赖关系，无需手动编写大量的配置代码，使得Spring的开发更加简洁和灵活。

总结来说，Spring的自动装配机制是通过类型、名称、构造器等方式自动注入依赖，从而简化了开发工作，减少了手动配置的麻烦，增强了系统的可维护性和灵活性。
...
- 子注解`@EnableAutoConfiguration`使用 @import注解，最终导入一个`AutoConfigurationImportSelector`组件来实现自动配置
- 这个组件中会通过`SpringFactories机制`去读取`META-INF/spring.factories`文件中的所有自动配置类。这是一种SPI机制
- 然后根据这些自动配置类上的条件注解`@Conditional`进行过滤，过滤出满足条件的自动配置类
- 然后通过Spring提供的`ImportSelector接口`来加载这些自动配置类
- 从而得到这些自动配置类的`BeanDefinition` - 最后根据BeanDefinition实例化对象



![image-20250626075135896](./assets/img40.png)



如何实现自动导入配置类

![image-20250626075641068](./assets/img41.png)



读取指定配置文件

![image-20250626075737672](./assets/img42.png)



获取配置的关键类

![image-20250626080021716](./assets/img43.png)



具体如下：

![image-20250626080049120](./assets/img44.png)



上面这个图总结如下

![image-20250626080649806](./assets/img45.png)



最终终结

![image-20250626080946994](./assets/img46.png)



#### Spring Boot Starter 的作用

Spring Boot Starter 是 Spring Boot 提供的一种简化配置和开发的机制。它的作用是通过预定义的一组配置和自动化设置，使得开发者能够更快速、简便地开始使用某个功能，而不需要手动配置所有细节。

主要作用：

1. **简化配置**：
   - Spring Boot Starter 提供了开箱即用的默认配置，使得你可以不用写很多复杂的配置代码。比如，你只需要引入某个 Starter 依赖，它会自动为你配置一些常用的设置，减少了繁琐的配置过程。

2. **快速集成功能**：
   - Starter 提供了与各种常用功能（如数据库、Web、消息队列等）的集成支持。通过引入相应的 Starter，Spring Boot 会自动为这些功能配置合适的默认设置和依赖。例如，`spring-boot-starter-web` 用于构建 Web 应用，`spring-boot-starter-data-jpa` 用于数据库操作。

3. **模块化的构建**：
   - Starter 可以看作是某一类功能的封装，它将一个模块的所有必要依赖和配置整合在一起。例如，通过 `spring-boot-starter-thymeleaf`，可以将 Thymeleaf 模板引擎的所有依赖和配置一次性引入。

4. **促进最佳实践**：
   - 每个 Starter 都是 Spring Boot 团队为特定功能模块所推荐的最佳配置，它会引导开发者使用 Spring Boot 的最佳实践，无需自行琢磨这些设置的细节。

5. **减少样板代码**：
   - 通过引入 Starter，开发者可以避免手动编写大量的基础设施代码。例如，在 Spring Boot 中引入 `spring-boot-starter-logging`，自动配置了日志功能，开发者无需显式配置日志框架。

示例：

- **`spring-boot-starter-web`**：用于创建 Web 应用，自动配置嵌入式 Tomcat 和 Spring MVC 等。
- **`spring-boot-starter-data-jpa`**：用于 JPA（Java Persistence API）数据库集成，自动配置 Hibernate 和 JPA 相关的设置。
- **`spring-boot-starter-test`**：包含测试所需的各种工具和库（如 JUnit、Mockito、Spring Test 等），方便进行单元测试。

使用方法：

只需要在 `pom.xml` 或 `build.gradle` 文件中引入对应的 Starter 依赖，Spring Boot 会自动进行依赖注入和配置。
...
Spring Boot Starter 的作用就是为开发者提供快速集成常见功能的方式，自动配置依赖和设置，减少了开发者手动配置的工作量，使得开发过程更加高效、简洁。
...
以下是一个Java中的动态代理案例，使用JDK的`Proxy`类和CGLIB库进行代理。

#### 1. 使用JDK动态代理

JDK动态代理要求目标类实现接口。下面是一个简单的示例。
        ...
        HelloService proxyInstance = (HelloService) Proxy.newProxyInstance(
                ...
                new InvocationHandler() {
                    ...
                    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                        ...
                        Object result = method.invoke(helloService, args); // 调用目标方法
...
CGLIB（Code Generation Library）是一个功能强大的字节码生成库，可以通过继承方式实现代理。与JDK代理不同，CGLIB不要求目标类实现接口。

##### 2.1 添加CGLIB依赖

如果你使用的是Maven，可以在`pom.xml`中添加CGLIB依赖：
...
public class CGLibProxyExample {
    public static void main(String[] args) {
        ...
        // 创建CGLIB代理
        HelloServiceImpl proxyInstance = (HelloServiceImpl) Enhancer.create(
...
- **JDK 动态代理**: 需要目标类实现接口，通过`Proxy.newProxyInstance`创建代理对象，适用于接口的代理。
- **CGLIB 动态代理**: 通过继承目标类来创建代理对象，不要求目标类实现接口，适用于没有接口的类。

JDK动态代理更常用于接口代理，而CGLIB则适用于无法通过接口代理的类，尤其在需要对类本身进行增强时很有用。
...
Java反射（Reflection）是Java的一项强大特性，它允许程序在运行时动态地查看和操作类、方法、字段等信息。反射为Java程序提供了更高的灵活性和可扩展性，但也带来了性能开销，因此需要谨慎使用。

#### 1. 反射的基本概念
反射使得Java程序能够在运行时获取类的结构信息，并能够操作对象的字段、方法等。例如，你可以在不事先知道类定义的情况下，通过反射来创建类的实例、访问成员变量、调用方法。

#### 2. 反射的常见应用场景
- **动态加载类**：通过反射可以在运行时加载和使用类，而不需要在编译时知道类的名称。
- **操作类的属性和方法**：可以动态访问类的字段、方法、构造函数等。
- **实现框架**：例如Spring、Hibernate等框架广泛使用反射来实现依赖注入、事务管理等功能。
- **开发工具**：反射可以用于开发工具或库，如代码生成器、ORM（对象关系映射）框架等。

#### 3. 反射的主要类
- **Class**：代表Java类的类型，提供了很多方法来获取类的信息，如`getName()`、`getDeclaredMethods()`、`getDeclaredFields()`等。
- **Constructor**：表示类的构造函数，可以通过反射动态地调用构造函数。
- **Field**：代表类的字段，可以通过反射访问、修改字段的值。
- **Method**：代表类的方法，通过反射可以动态调用方法。
- **Modifier**：提供对字段、方法、类的访问修饰符（如public、private等）进行处理的工具。

#### 4. 使用反射的示例

##### 4.1 获取Class对象
```java
Class<?> clazz = Class.forName("java.util.ArrayList"); // 获取Class对象
```
...
int size = (int) field.get(obj); // 获取字段值
field.set(obj, 100); // 修改字段值
```

##### 4.4 调用方法
```java
Method method = clazz.getDeclaredMethod("add", Object.class);
method.invoke(obj, "Hello, World!"); // 调用add方法
```

##### 5. 反射的性能问题
反射虽然强大，但它在运行时需要动态解析，因此比直接调用方法或访问字段要慢得多。频繁使用反射可能会导致程序性能下降。

##### 6. 反射的安全性问题
反射可以绕过Java的访问控制机制，访问私有成员变量和方法。这意味着反射可能会使得程序更加容易受到攻击。因此，在使用反射时需要特别注意安全性问题，确保不会泄露敏感信息。

##### 7. 总结
反射是Java中一项非常强大的特性，它为程序提供了动态操作的能力，但由于其性能和安全性问题，需要在使用时谨慎权衡。通常建议只在必要的情况下使用反射，例如框架开发、工具开发等。



==Mybatis==

我在学习mybatis基于springboot使用（侧重于使用），并帮我总结一份教程，至少包含以下内容（其他重要内容如果我没有指出，请补充），教程的内容顺序，要求你自行排版调整，要求循序渐进

1、基于spring boot 使用 mybatis 进行增删改查的案例（使用Mapper.xml的方式指定sql，而不是@Insert等注解）

2、resultMap处理字段和列不一致问题
...
7、Mybatis 动态SQL（常用的标签都要介绍）

8、如果查询的对象，包含了其他对象，怎么处理

- 比如：user 表里包含了一个class 对象（1对1）
- 比如：class表里包含了多个user对象（1多多）



==Spring Boot==

我在学习mybatis基于springboot 3使用（侧重于使用），并帮我总结一份教程，至少包含以下内容（其他重要内容如果我没有指出，请补充），教程的内容顺序，要求你自行排版调整，要求循序渐进

- 案例基于 MySQL、Mybatis、Spring Boot，Spring Boot、Redis（频繁查询的接口进行缓存） 配置文件使用 yml 的 CRUD 案例
- 对于Mybatis，要求使用 XxxMapper.xml 的形式，并且要求sql健壮（使用if,where, set等标签防止参数有问题）
- 接口基于restful api
- 前端传过来的参数用对应的DTO进行封装与返回
- 要求使用 spring boot 的 validation 模块进行参数的校验（最好每个参数都校验）
- 后端返回给前端的数据，进行统一的数据格式封装
- 如果后端发生异常，进行统一统一拦截，进行统一的数据格式封装，并返回具体的错误信息（比如：code=400，message="用户名不能为空"，data=null）



==常用注解==

帮我总结 Spring Boot 常用注解（ 包含Spring）简要介绍作用与用法，包含以下方面（如果我没举例，但是很重要的，请补充）
...
根据文章的id查询，cms_information表中文章的详情信息并返回。

B：查询当前文章的点赞、收藏、转发的数量，以及查询当前用户是否对当前文章点赞、收藏以及转发。

具体实现：

首先查询redis中的set集合

key：为前缀+文章id，例如点赞 like_001、收藏 favor_001、转发 forward_001
...
如果redis中查询不到，则查询数据库表中的数据。



包括点赞、收藏、评论等功能的实现。技术要点1:使用 redis 缓存的 set 集合数据结构存储用户点赞、收藏、转发等行为以及总量，避免高并发情况下，总量统计不准确等问题。

使用 redis 缓存的 set 集合数据结构存储用户点赞、收藏、转发等行为以及总量，避免高并发情况下，总量统计不准确等问题。

技术要点2:redis 分布式锁与spring 本地事务存在锁提前释放，事务后提交或回滚问题的解决。

--------------------------------------------------------------------------------------------------------------------------------



我想将断场景与解决方案写到我的java开发工程师简历中，作为项目亮点。但是我不熟悉这个业务场景。帮我总结出这个场景以及实现方法（从mysql表设计到后端java实现）
